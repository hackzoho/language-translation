//Sabbineni Mohan Aditya-17045087-Chemical
//PA2
...
vector <string> P = { "243f6a88", "85a308d3", "13198a2e", "03707344", "a4093822",
                   "299f31d0", "082efa98", "ec4e6c89", "452821e6", "38d01377",
                   "be5466cf", "34e90c6c", "c0ac29b7", "c97c50dd", "3f84d5b5",
                   "b5470917", "9216d5d9", "8979fb1b" };

//////////////////////////////////////////////////////////////////////////////////////////////
// Function for decimal to binary (STRING OUTPUT)
string decToBinary(long long n)
{
    string s=""; // array to store binary number
    while (n > 0)
    {
        s += char(n % 2 + 48);// storing remainder in binary array(Considered ASCII Value and added the same)
        n = n / 2;
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////
//Function for Hex to Bin (STRING OUTPUT)
string hexToBin(string plaintext)
{
    ...
    for(int i=0; i<n;i++)
    {
        x=plaintext.at(i);
        num=  stol(x,0,16);
        bin4b=decToBinary(num);
        bin4b="0000"+bin4b;
        bin4b=bin4b.substr(bin4b.size()-4);
        bin=bin+bin4b;
    }
    return bin;
}
...
//////////////////////////////////////////////////////////////////////////////////////////////
//Function for Bin to Hex (STRING OUTPUT)
...
//////////////////////////////////////////////////////////////////////////////////////////////
// xor of two hexadecimal strings of same length.
 string xori(string a, string b)
    {
        ...
        ans = binToHex(ans);
        while (ans.size() < 8)
            ans = "0" + ans;//to maintain output length of 32 bit
        return ans;
    }
//////////////////////////////////////////////////////////////////////////////////////////////
// to store 2^32(for addition modulo 2^32).
long long modVal = 4294967296; //2^32 = 4294967296
//////////////////////////////////////////////////////////////////////////////////////////////
// addition of modulo 2^32 of two hexadecimal strings.
string addBin(string a, string b)
    {
        long long n1 = stoll(a,0,16); //converting string to long long type
        long long n2 = stoll(b,0,16);
        n1 = (n1 + n2) % modVal;
        stringstream ss;
        ss << hex << n1; //converting to hex
        string ans = ss.str();
        ans = "00000000" + ans;
...
string f(string plainText)
    {
      vector <string> a(4);
        ...
        while(i<8)
        {
            int col= stoi( hexToBin(plainText.substr(0,2)),0,2);
            a[i/2] = S[i/2][col];
            plainText.erase(0,2);
            i=i+2;
        }
...
//////////////////////////////////////////////////////////////////////////////////////////////
//Function for generating subkeys.
    void keyGenerate(string key)
    {
        int j = 0;
        for (int i = 0; i < P.size(); i++) {
            P[i] = xori(P[i], key.substr(j, j + 8));
            j = (j + 8) % key.size();
        }
    }
//////////////////////////////////////////////////////////////////////////////////////////////
// round function
    string round(int time, string plainText)
    {
        ...
        left = xori(left, P[time]);
        string fOut = f(left); // output from F function
        right = xori(fOut, right);
        return right + left;
    }
//////////////////////////////////////////////////////////////////////////////////////////////
// encryption
    string encrypt(string plainText)
    {
        ...
        // postprocessing
        string right = plainText.substr(0, 8);
        string left = plainText.substr(8, 16);
        right = xori(right, P[16]);
        left = xori(left, P[17]);
        return left + right;
    }
//////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////
// char to ascii value
string charToAscii(string plaintext)//Takes single character!!!
{
    string char_ascii="";
    char_ascii =to_string(int(plaintext[0]));
    return char_ascii;
}

//////////////////////////////////////////////////////////////////////////////////////////////
// char to hex value [directly]
string charToHex(string plaintext)//takes single character!!!!
{
    string char_ascii="";
    char_ascii=charToAscii(plaintext);
    int val =stoi(char_ascii);
    stringstream ss;
    ss<< hex<< val;
    string char_hexa= ss.str();
    return char_hexa;
}

//////////////////////////////////////////////////////////////////////////////////////////////
//Text to hex value
string textToHex(string plaintext)//Takes whole text!!!
{
    ...
    for (int i=0;i<n;i++)
    {
        place=plaintext.at(i);
        ascii_to_hex=charToHex(place);
        pt_hex=pt_hex.append(ascii_to_hex);
    }
    return pt_hex;
}
...
//////////////////////////////////////////////////////////////////////////////////////////////
// xor function for CBC
 string xorcbc(string a, string b)
    {
        a = hexToBin(a);
        b = hexToBin(b);
        string ans = "";
        ...
        ans = binToHex(ans);
        while (ans.size() < 16)
            ans = "0" + ans;//to maintain output length of 64 bit
        return ans;
    }

//////////////////////////////////////////////////////////////////////////////////////////////
//Block function for first block encryption
string BEFirst(string iv, string plaintext)
{
    string xored_text_iv = xorcbc(iv,plaintext);
    string block_cipher=blowfishencrypt(xored_text_iv);
    cout << "Cipher out after block-1: "<< block_cipher<<endl;
    return block_cipher;/**Output Cipher text done**/
}

//////////////////////////////////////////////////////////////////////////////////////////////
//Block function for middle blocks encryption
string BEMid(int i,string cipher_iv, string plaintext)
{
    string xored_text_iv = xorcbc(cipher_iv,plaintext);
    string block_cipher=blowfishencrypt(xored_text_iv);
    cout<<"Cipher out after block-"<<i<<": "<<block_cipher<<endl;
    return block_cipher;/**Output Cipher text done**/
}

//////////////////////////////////////////////////////////////////////////////////////////////
//Block function for last block encryption
string BELast(string cipher_iv, string plaintext)
{
   //Padding of text
   string padded_pt="";
    int n= plaintext.size();
    int p = 16-n;
    string psize=to_string(p/2);
    if (p%2==0)
    {
        for(int i=0; i<p;i++)
        {
            if(i!=(p-1))
                plaintext=plaintext.append("0");
            else
                plaintext=plaintext.append(psize);
        }
    }
    else
    {
    ...
    string xored_text_iv = xorcbc(cipher_iv,plaintext);
    string block_cipher=blowfishencrypt(xored_text_iv);
    cout << "Cipher out after last block: "<< block_cipher<<endl;
     return block_cipher;

}

//////////////////////////////////////////////////////////////////////////////////////////////
//Parent function for CBC mode
string CBCencrypt(string plaintext)
    {
        ...
        cout<<"Plain text sixe: "<<n<<endl;
        string iv="";
        //Creating IV
        while(iv.size()<=8)
        {
            string ap=to_string(rand());
            iv=iv.append(textToHex(ap));
        }
        string cipher_iv=iv.substr(0,16);
        cout << "IV generated: "<<cipher_iv<<endl;
        ...
        if(n%16 == 0)
            ...
            blocks = (n/16)+1;
//Initialising the loop by breaking the blocks
        for( int i=0; i< blocks; i++)
        {
            if(i==0&&hex_plaintext.size()>=16)
            {
                string plaintext_block = hex_plaintext.substr(0,16);
                cipher_iv=BEFirst(cipher_iv,hex_plaintext.substr(0,16));
                output_cipher=cipher_iv;
                hex_plaintext.erase(0,16);
            }
            else if(hex_plaintext.size()>=16)
            {
                string plaintext_block = hex_plaintext.substr(0,16);
                string blockout_cipher="";
                blockout_cipher=BEMid(i+1,cipher_iv,plaintext_block);
                cipher_iv=blockout_cipher;
            ...
            {
                string plaintext_block = hex_plaintext;
                string blockout_cipher="";
                blockout_cipher=BELast(cipher_iv,plaintext_block);
                cipher_iv=blockout_cipher;
                output_cipher=output_cipher.append(blockout_cipher);
        ...
        cout << " "<< endl;
        cout<<"CBC Encryption SUMMARY--------"<<endl;
        cout << "Plain Text: "<<plaintext<<endl;
        cout<<"Output Cipher Text by CBC Encrypt(HEX): "<<(output_cipher)<<endl;
        cout<<"Output Cipher Text by CBC Encrypt: "<<hexToASCII(output_cipher)<<endl;
        cout << " "<< endl;
        cout << " ------------------------------------------------------------------------"<< endl;
...
//////////////////////////////////////////////////////////
//XOR function for OFB mode
string xorofb(string iv, string b) //HexInput
    {
        iv = hexToBin(iv);
        b = hexToBin(b);
        string ans = "";
        for (int i = 0; i < b.size(); i++)
            ans += (char)(((iv[i] - '0') ^ (b[i] - '0')) + '0');
        ans = binToHex(ans);
        return ans;
    }
...
string OFBencrypt(string plaintext)
{
    cout << "OFB Encrypt MODULE START--------"<<endl;
    cout << "Plain Text received by OFB Encrypt: "<<plaintext<<endl;
    string hex_plaintext = textToHex(plaintext);
    int n = hex_plaintext.size();
    cout<<"Plain text sixe: "<<n<<endl;
    string block_iv="";
    string iv="";
        ...
        iv=textToHex(iv).substr(0,16);
        cout << "Nonce generated: "<<iv<<endl;
        string output_cipher="";


    int s= 16; //default stream set size=16(maximum size)
    int blocks; //Stream of 16 hex
        if(n%s == 0)
            blocks = n/s;
        else
            blocks = (n/s)+1;
    for( int i=0; i< blocks; i++)
    {
        if(i==0)
        {
            string plaintext_block = hex_plaintext.substr(0,s);
            block_iv=blowfishencrypt(iv); // sending nonce to blowfish algorithm
            output_cipher=xorofb(block_iv,plaintext_block); // xoring to get the cipher
            cout<<"Cipher out after block-"<<i+1<<": "<<output_cipher<<endl;
            hex_plaintext.erase(0,s);
        }
        else
        {
            string plaintext_block = hex_plaintext.substr(0,s);
            block_iv=blowfishencrypt(block_iv);//using the value from the previous block
            string blockout_cipher="";
            blockout_cipher=xorofb(block_iv,plaintext_block);
            cout<<"Cipher out after block-"<<i+1<<": "<<blockout_cipher<<endl;
            output_cipher=output_cipher.append(blockout_cipher);
            hex_plaintext.erase(0,s);
        }
    }
        cout << " "<< endl;
        cout<<"OFB Encrypt SUMMARY==="<<endl;
        cout << "Plain Text: "<<plaintext<<endl;
        cout<<"Output Cipher Text by OFB Encrypt(HEX): "<<(output_cipher)<<endl;
        cout<<"Output Cipher Text by OFB Encrypt: "<< hexToASCII(output_cipher)<<endl;
        cout << " "<< endl;
        cout << " ------------------------------------------------------------------------"<< endl;
        return output_cipher;
}
...
{
      string plaintext="";
    plaintext = "17045087_Testing CBC mode_PA2";
    CBCencrypt(plaintext);
     OFBencrypt(plaintext);
}
